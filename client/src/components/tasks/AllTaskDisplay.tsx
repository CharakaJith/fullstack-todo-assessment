  ...
  const fetchTasks = async () => {
    try {
      ...
      if (response.success) {
        ...
        setError([response.response.data.message || 'Failed to fetch tasks']);
      }
    } catch (error: any) {
    ...
    <div className="flex flex-col gap-4 w-full h-full">
      ...
      {tasks.length === 0 ? (
        ...
        <div className="flex flex-col gap-3 overflow-auto h-full pr-2 pt-1 md:pt-7 pb-1 md:pb-7">
          {tasks.map((task) => (
            // task box
            <div key={task.id} className="w-full border border-gray-300 rounded-lg p-4 shadow-sm flex flex-col">
              {/* task details */}
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                ...
                <div className="flex gap-2 w-full md:w-1/2">
                  ...
                  <Button className="flex-1 px-0 py-1 text-xs md:text-sm bg-red-600 text-white rounded hover:bg-red-800 cursor-pointer text-center">
                    Delete
                  </Button>
                </div>
              </div>
